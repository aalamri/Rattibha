...
import { DISPUTE_RESOLUTION, LEGAL_LAST_UPDATED } from '@/content/legalPages';
import { SUPPORTED_LANGUAGES, type AppLanguage } from '@/i18n/constants';
import { buildLegalMetadata } from '@/lib/seo';

const PATH = '/dispute-resolution';
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return buildLegalMetadata(lang, PATH, DISPUTE_RESOLUTION[lang]);
}
...
export default async function DisputeResolutionPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <>
      <NavBar />
      <LegalPageBody content={DISPUTE_RESOLUTION[lang]} lastUpdated={LEGAL_LAST_UPDATED[lang]} />
      <Footer />
    </>
  );
}
